...
function Page() {
  ...
  const [statusFilter, setStatusFilter] = useState<"submitted" | "under_review" | "approved" | "rejected" | "revision_required" | "">("submitted");
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)} className="h-9 rounded-md border border-border bg-background px-2 text-sm">
      ...
      {openId && <ReviewDialog id={openId} onClose={() => { setOpenId(null); load(); }} />}
    </div>
  );
}
